    ...
    switch ($section) {
        ...
        case 'penghasilan_pegawai':
            ...
            if ($jenis_pekerjaan_post === 'pppk') {
                ...
                // Gunakan helper function untuk hitung repayment capacity
                $repayment_capacity = hitung_repayment($gaji_pp, $biaya_hidup, $cic);
                
                // Terapkan safety margin (95%) - conservative approach
                $rpc = max(0, $repayment_capacity * 0.95);
                ...
                $repayment_capacity = hitung_repayment($omset_total, $biaya_hidup, $cic);
                $rpc = max(0, $repayment_capacity * 0.95);
        ...
        case 'usaha':
            ...
            // G. Repayment Capacity (95% konservatif)
            $rpc = $net_cashflow * 0.95;
        ...
        case 'neraca':
            ...
            $stmt = $pdo->prepare("INSERT INTO analisa_neraca 
                (id_pengajuan, aktiva_kas, aktiva_tabungan, aktiva_tanah, aktiva_kendaraan, aktiva_stok, aktiva_lainnya, 
                 pasiva_hutang_bank, pasiva_hutang_lain, pasiva_modal, total_aktiva, total_pasiva)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$id_pengajuan, $n_kas, $n_bank, $n_tanah, $n_kend, $n_stok, $n_lain, $n_hutang_bank, $n_hutang_lain, $n_modal, $total_aktiva, $total_pasiva]);
            ...
            // 📋 Log activity untuk audit trail
            log_activity($pdo, $_SESSION['user_id'] ?? 0, 
                "Menyimpan Data Neraca (ID Pengajuan: $id_pengajuan | Total Aktiva: " . 
                number_format($total_aktiva, 0, ',', '.') . " | Total Pasiva: " . 
                number_format($total_pasiva, 0, ',', '.') . ")");

            echo json_encode(['success' => true, 'message' => 'Neraca berhasil disimpan!']);
            break;
        ...
        case '6c':
            ...
            try {
                ...
                // Rekomendasi otomatis berdasarkan klasifikasi
                $rekomendasi = $_POST['rekomendasi_6c'] ?? '';
                if (empty($rekomendasi)) {
                    // Auto-recommend based on classification
                    if ($rata_rata_score <= 2.5) {
                        $rekomendasi = 'LAYAK';
                    } elseif ($rata_rata_score <= 3.5) {
