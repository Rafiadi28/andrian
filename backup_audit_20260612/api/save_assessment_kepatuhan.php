...
try {
    ...
    $fasilitas = [];
    if (isset($_POST['fasilitas_rek']) && is_array($_POST['fasilitas_rek'])) {
        foreach ($_POST['fasilitas_rek'] as $i => $rek) {
            $rek = trim($rek);
            if (!empty($rek)) {
                $fasilitas[] = [
                    'rek' => $rek,
                    'tgl' => trim($_POST['fasilitas_akad'][$i] ?? ''),
                    'jt' => trim($_POST['fasilitas_jtempo'][$i] ?? ''),
                    'kol' => trim($_POST['fasilitas_kol'][$i] ?? ''),
                    'plafond' => str_replace(',', '', trim($_POST['fasilitas_plafond'][$i] ?? '')) ?: '0',
                    'saldo' => str_replace(',', '', trim($_POST['fasilitas_saldo'][$i] ?? '')) ?: '0',
                ];
            }
        }
    }
