    ...
    if (!verifyCsrfToken($_POST['csrf_token'] ?? '')) {
        ...
        if ($jenis_jaminan == 'tanah_bangunan') {
            ...
            if ($masa_berlaku_stnk) {
                ...
                if (!$dateCheck) {
                    throw new Exception('Format masa berlaku STNK tidak valid.');
                }
            }

            $stmt = $pdo->prepare("INSERT INTO jaminan_kendaraan (id_pengajuan, merk, tipe, tahun_pembuatan, no_polisi, no_rangka, no_mesin, nama_pemilik, nilai_pasar, nilai_taksasi, nilai_likuidasi, tipe_valuasi, nilai_taksasi_manual, no_stnk, masa_berlaku_stnk) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([
                $id_pengajuan,
                $_POST['merk'],
                $_POST['tipe'],
...
<head>
    ...
    <script>
        ...
        function calcKendaraan() {
            ...
            let tipeValuasi = document.querySelector('[name=tipe_valuasi_kendaraan]')?.value || 'otomatis';
            if (tipeValuasi === 'manual') {
                let manualValue = parseFloat(document.querySelector('[name=nilai_taksasi_manual_kendaraan]')?.value) || 0;
                if (manualValue > 0) {
                    nilaiTaksasi = manualValue; // Use manual value if provided
                }
            }
        ...
        function toggleTaksasiManualKendaraan() {
            let tipeElem = document.querySelector('[name=tipe_valuasi_kendaraan]');
            if(!tipeElem) return;
            let val = tipeElem.value;
            
            let manualWrap = document.getElementById('wrap_nilai_taksasi_manual_kendaraan');
            if (manualWrap) {
                if (val === 'manual') {
                    manualWrap.style.display = 'block';
                } else {
                    manualWrap.style.display = 'none';
                    let manualInput = document.querySelector('[name=nilai_taksasi_manual_kendaraan]');
        ...
        function calcTanah() {
            ...
            // Check if manual override is selected
            let tipeValuasi = document.querySelector('[name=tipe_valuasi_tanah]')?.value || 'otomatis';
            if (tipeValuasi === 'manual') {
                let manualValue = parseFloat(document.querySelector('[name=nilai_taksasi_manual_tanah]')?.value) || 0;
                if (manualValue > 0) {
                    taksasi = manualValue; // Use manual value if provided
                }
            }
        ...
        function toggleTaksasiManualTanah() {
            let tipeElem = document.querySelector('[name=tipe_valuasi_tanah]');
            if(!tipeElem) return;
            let val = tipeElem.value;
            
            let manualWrap = document.getElementById('wrap_nilai_taksasi_manual_tanah');
            if (manualWrap) {
                if (val === 'manual') {
                    manualWrap.style.display = 'block';
                } else {
                    manualWrap.style.display = 'none';
                    let manualInput = document.querySelector('[name=nilai_taksasi_manual_tanah]');
                    if (manualInput) manualInput.value = '';
                }
            }
            calcTanah();
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (document.querySelector('[name=jenis_surat]')) {
                ...
                // Initialize tipe valuasi toggle
                toggleTaksasiManualTanah();
            }
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <?php if ($jenis_jaminan == 'tanah_bangunan'): ?>
                    ...
                    <div class="grid-2">
                        ...
                        <div class="form-group">
                            <label>Tipe Valuasi Taksasi</label>
                            <select name="tipe_valuasi_tanah" id="tipe_valuasi_tanah" onchange="toggleTaksasiManualTanah()">
                    ...
                    <div class="form-group" id="wrap_nilai_taksasi_manual_tanah" style="display:none;">
                        <label>Nilai Taksasi Manual (Rp) - Untuk Override Nilai Otomatis</label>
                        <input type="number" name="nilai_taksasi_manual_tanah" oninput="calcTanah()" placeholder="Masukkan nilai taksasi manual">
                        <small style="color:#666; margin-top:0.25rem; display:block;">Jika diisi, nilai ini akan menggantikan perhitungan otomatis.</small>
                    </div>
                ...
                <?php elseif ($jenis_jaminan == 'kendaraan'): ?>
                    ...
                    <div class="grid-2">
                        ...
                        <div class="form-group">
                            <label>Tipe Valuasi Taksasi</label>
                            <select name="tipe_valuasi_kendaraan" id="tipe_valuasi_kendaraan" onchange="toggleTaksasiManualKendaraan()">
                                <option value="otomatis" selected>🔄 Otomatis</option>
                                <option value="manual">✏️ Manual Override</option>
                            </select>
                        </div>
                    ...
                    <div class="form-group" id="wrap_nilai_taksasi_manual_kendaraan" style="display:none;">
                        <label>Nilai Taksasi Manual (Rp) - Untuk Override Nilai Otomatis</label>
                        <input type="number" name="nilai_taksasi_manual_kendaraan" oninput="calcKendaraan()" placeholder="Masukkan nilai taksasi manual">
                        <small style="color:#666; margin-top:0.25rem; display:block;">Jika diisi, nilai ini akan menggantikan perhitungan otomatis.</small>
                    </div>
                    <div style="background:#f0abfc; padding:1.5rem; border-radius:8px; margin-top:1.5rem; color:#86198f;">
